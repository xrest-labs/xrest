...
const handleSaveRequest = async (payload: { serviceIndex: number, updatedItem: any, tab: any }) => {
  try {
    await collectionsStore.updateCollection(payload.serviceIndex, payload.updatedItem)
  ...
  <div class="flex-1 overflow-hidden h-full">
    <ResizablePanelGroup direction="horizontal">
      ...
      <ResizablePanel :default-size="80" class="flex flex-col h-full">
        <RequestWorkspace v-model="activeTab" :items="collectionsStore.collections" label="Collection"
          @share-request="handleShareRequest" @save-request="handleSaveRequest" @update-item="handleUpdateItem"
          @delete-item="handleDeleteItem" @reload-items="collectionsStore.loadCollections" />
      </ResizablePanel>
    </ResizablePanelGroup>
