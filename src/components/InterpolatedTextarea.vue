...
<template>
  <div class="relative w-full group overflow-hidden rounded-md border bg-background transition-shadow focus-within:ring-1 focus-within:ring-primary">
    <!-- 1. Text Layer -->
    <div
      ref="textRef"
    ...
    <textarea
      ...
      :class="cn(
        'w-full min-h-[150px] bg-transparent border-none p-2 focus:outline-none resize-y relative z-10',
                ...
                <div class="space-y-1.5 p-0.5">
                  <div v-if="token.isValid" class="flex flex-col gap-1">
                    <div class="flex items-center justify-between gap-4">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Active Value</span>
                      </div>
                      <span v-if="environmentName" class="text-[9px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground font-bold border border-border/50">{{ environmentName }}</span>
                    </div>
                    <span class="font-mono text-xs text-primary bg-primary/5 p-1.5 rounded border border-primary/10 break-all select-all">{{ token.resolvedValue }}</span>
                  </div>
                  <div v-else class="flex flex-col gap-1 text-destructive">
                    <div class="flex items-center justify-between gap-4">
                      ...
                      <span v-if="environmentName" class="text-[9px] bg-destructive/10 px-1.5 py-0.5 rounded text-destructive font-bold border border-destructive/20">{{ environmentName }}</span>
                    </div>
                    <span class="text-[11px] leading-relaxed">The variable <code class="font-bold">{{ token.content }}</code> is not defined in the current environment.</span>
                  </div>
                </div>
              </TooltipContent>
            </Tooltip>
