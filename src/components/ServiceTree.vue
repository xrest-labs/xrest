...
import { ChevronRight, ChevronDown, Layers, Settings, Check, ChevronDown as ChevronDownIcon } from 'lucide-vue-next'
...
watch(() => props.services, (newServices) => {
  localServices.value = newServices.map(ns => {
    ...
    return {
      ...ns,
      isOpen: forceOpen !== undefined ? forceOpen : (existing ? existing.isOpen : true)
    }
  })
}, { deep: true })
    ...
    <div v-for="(service, sIdx) in localServices" :key="service.id" class="select-none">
        ...
        <Popover v-if="shouldShowEnvironments">
          <PopoverTrigger as-child>
            <button v-if="service.environments && service.environments.length > 0" :class="[
              ...
              isEnvUnsafe(service, service.selectedEnvironment || service.environments[0].name)
          ...
          <PopoverContent class="w-28 p-1" align="end">
            <div class="flex flex-col gap-0.5">
              <button v-for="env in getEnvironments(service)" :key="env" @click="handleEnvChange(service.id, env)"
                :class="[
                  'flex items-center justify-between px-2 py-1.5 text-[10px] rounded-sm transition-colors',
                  (service.selectedEnvironment || service.environments[0].name) === env
                    ? (isEnvUnsafe(service, env) ? 'bg-destructive/10 text-destructive font-bold' : 'bg-primary/10 text-primary font-bold')
                    : (isEnvUnsafe(service, env) ? 'text-destructive/70 hover:bg-destructive/5 hover:text-destructive' : 'hover:bg-muted text-muted-foreground hover:text-foreground')
                ]">
                <span>{{ env }}</span>
                <Check v-if="(service.selectedEnvironment || service.environments[0].name) === env" class="h-3 w-3" />
              </button>
            </div>
          </PopoverContent>
        </Popover>
      ...
      <div v-if="service.isOpen" class="ml-4 mt-0.5 space-y-0.5 border-l border-border/50 pl-2">
        <div v-for="endpoint in service.endpoints" :key="endpoint.id" @click="emits('select-endpoint', endpoint)"
          ...
          <div :class="['text-[10px] font-bold w-10 shrink-0 capitalize text-right', getMethodColor(endpoint.method)]">
            {{ endpoint.method }}
          </div>
          <span class="text-xs text-foreground/80 group-hover:text-foreground truncate">{{ endpoint.name }}</span>
        </div>
      </div>
    </div>
  </div>
