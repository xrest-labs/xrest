...
const tokens = computed(() => parseInterpolation(props.modelValue || '', props.variables || {}))
...
const handleKeyDown = (e: KeyboardEvent) => {
  ...
  if (e.key === 'ArrowDown') {
    e.preventDefault()
    autocompleteIndex.value = (autocompleteIndex.value + 1) % filteredVariables.value.length
  } else if (e.key === 'ArrowUp') {
    e.preventDefault()
    autocompleteIndex.value = (autocompleteIndex.value - 1 + filteredVariables.value.length) % filteredVariables.value.length
...
onMounted(() => {
  syncScroll()
  window.addEventListener('scroll', () => { if (showAutocomplete.value) showAutocomplete.value = false }, true)
  window.addEventListener('mousedown', handleClickOutside)
})

onUnmounted(() => {
  window.removeEventListener('scroll', () => { if (showAutocomplete.value) showAutocomplete.value = false }, true)
  ...
  <div
    ...
    <div ref="textRef"
      ...
      <template v-for="(token, i) in tokens" :key="i">
        <span v-if="token.type === 'text'" class="text-foreground">{{ token.content }}</span>
        <span v-else class="text-transparent selection:text-transparent">{{ token.content }}</span>
      </template>
    </div>
    ...
    <div ref="highlightRef"
      ...
      :style="{ fontSize: 'inherit' }">
      <template v-for="(token, i) in tokens" :key="i">
        <span v-if="token.type === 'text'" class="text-transparent">{{ token.content }}</span>
                  ...
                  <div v-if="token.isValid" class="flex flex-col gap-1">
                    <div class="flex items-center justify-between gap-4">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Active
                          Value</span>
                      </div>
                      <span v-if="environmentName"
                        class="text-[9px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground font-bold border border-border/50">{{
                          environmentName }}</span>
                    </div>
                    <span
                      class="font-mono text-xs text-primary bg-primary/5 p-1.5 rounded border border-primary/10 break-all select-all">{{
                  ...
                  <div v-else class="flex flex-col gap-1 text-destructive">
                    <div class="flex items-center justify-between gap-4">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] uppercase font-bold tracking-wider">Missing Variable</span>
                      </div>
                      <span v-if="environmentName"
                        class="text-[9px] bg-destructive/10 px-1.5 py-0.5 rounded text-destructive font-bold border border-destructive/20">{{
                          environmentName }}</span>
                    </div>
                    <span class="text-[11px] leading-relaxed">The variable <code
                        class="font-bold">{{ token.content }}</code> is not defined in the current environment.</span>
                  </div>
                </div>
              </TooltipContent>
            </Tooltip>
        ...
        <div class="p-1 max-h-[300px] overflow-auto">
          <div v-for="(varName, idx) in filteredVariables" :key="varName" @mouseenter="autocompleteIndex = idx"
            @mousedown.prevent="selectVariable(varName)" :class="cn(
              ...
              idx === autocompleteIndex ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/50'
            )">
            <div class="flex items-center gap-2">
              <div class="h-2 w-2 rounded-full bg-primary/40"></div>
              <span class="font-mono font-bold">{{ varName }}</span>
            </div>
            <span class="text-[10px] text-muted-foreground opacity-50 truncate max-w-[100px]">{{ variables?.[varName]
            }}</span>
          </div>
        </div>
        <div class="border-t bg-muted/30 px-2 py-1 flex items-center justify-between">
          <span class="text-[9px] text-muted-foreground uppercase font-bold tracking-widest">Environment
            Variables</span>
          <span class="text-[9px] text-muted-foreground opacity-50">{{ filteredVariables.length }} found</span>
        </div>
      </div>
    </Teleport>
  </div>
