  ...
  <div class="space-y-8 pb-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
    ...
    <div class="space-y-4">
      <div class="flex items-center gap-2 border-b pb-2">
        ...
        <h3 class="text-xs font-semibold uppercase tracking-wider">Authentication Configuration</h3>
      </div>

      <div class="grid grid-cols-[120px_1fr] items-center gap-4">
        <Label class="text-[11px] text-muted-foreground">Auth Type</Label>
        <Select v-model="auth.type">
          <SelectTrigger class="h-8 text-xs w-48 font-medium">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            ...
            <SelectItem value="none" class="text-xs">No Auth</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div v-if="auth.type === 'bearer'"
        ...
        <InterpolatedInput v-model="auth.bearerToken" :variables="variables" :environment-name="environmentName"
          placeholder="eyJhbGciOiJIUzI1..." class="h-8 text-[9px] font-mono bg-muted/20" />
      </div>
      ...
      <template v-if="auth.type === 'basic'">
        <div
          ...
          <InterpolatedInput v-model="auth.basicUser" :variables="variables" :environment-name="environmentName"
            placeholder="Username" class="h-8 text-[9px] bg-muted/20" />
        </div>
        <div
          class="grid grid-cols-[120px_1fr] items-center gap-4 animate-in fade-in slide-in-from-left-2 transition-all">
          ...
          <InterpolatedInput v-model="auth.basicPass" :variables="variables" :environment-name="environmentName"
            placeholder="Password" class="h-8 text-[9px] bg-muted/20" />
        </div>
      </template>
      ...
      <template v-if="auth.type === 'apikey'">
        <div
          class="grid grid-cols-[120px_1fr] items-center gap-4 animate-in fade-in slide-in-from-left-2 transition-all">
          <Label class="text-[11px] text-muted-foreground">Key Name</Label>
          <InterpolatedInput v-model="auth.apiKeyName" :variables="variables" :environment-name="environmentName"
            placeholder="X-API-Key" class="h-8 text-[9px] font-mono bg-muted/20" />
        </div>
        <div
          class="grid grid-cols-[120px_1fr] items-center gap-4 animate-in fade-in slide-in-from-left-2 transition-all">
          <Label class="text-[11px] text-muted-foreground">Key Value</Label>
          <InterpolatedInput v-model="auth.apiKeyValue" :variables="variables" :environment-name="environmentName"
            placeholder="Value" class="h-8 text-[9px] font-mono bg-muted/20" />
        </div>
        <div
          class="grid grid-cols-[120px_1fr] items-center gap-4 animate-in fade-in slide-in-from-left-2 transition-all">
          <Label class="text-[11px] text-muted-foreground">Add to</Label>
          <Select v-model="auth.apiKeyLocation">
            <SelectTrigger class="h-8 text-xs w-32">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="header" class="text-xs">Header</SelectItem>
              <SelectItem value="query" class="text-xs">Query Params</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </template>
    ...
    <div class="space-y-4 pt-4">
      <div class="flex items-center gap-2 border-b pb-2">
        ...
        <h3 class="text-xs font-semibold uppercase tracking-wider">Authentication Pre-flight</h3>
        <button
          class="ml-auto flex items-center gap-1.5 px-3 py-1 bg-primary/10 text-primary rounded-full text-[10px] font-bold border border-primary/20 hover:bg-primary/20 transition-all">
      ...
      <div v-if="preflight.enabled"
        ...
        <div class="space-y-3">
          <Label class="text-[10px] font-bold uppercase text-muted-foreground">Request Details</Label>
          <div class="flex items-center gap-2 bg-muted/50 p-2 rounded border">
            <Select v-model="preflight.method">
              <SelectTrigger class="w-24 h-8 text-[10px] font-bold bg-background">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <InterpolatedInput v-model="preflight.url" :variables="variables" :environment-name="environmentName"
              placeholder="Pre-flight URL (e.g. /oauth/token)" class="flex-1 h-8 text-[9px] font-mono bg-background" />
          </div>
        ...
        <div class="grid grid-cols-2 gap-4 pt-2 border-t mt-2">
          <div class="space-y-2">
            <Label class="text-[10px] font-bold uppercase text-muted-foreground">Token Extraction</Label>
            <div class="space-y-1.5">
              <Label class="text-[9px] text-muted-foreground">Token Key (Response JSON)</Label>
              <Input v-model="preflight.tokenKey" class="h-7 text-[9px] font-mono" placeholder="access_token" />
            </div>
            <div class="space-y-1.5">
              <Label class="text-[9px] text-muted-foreground">Token Header Name</Label>
              <Input v-model="preflight.tokenHeader" class="h-7 text-[9px] font-mono" placeholder="Authorization" />
            </div>
          </div>

          <div class="space-y-2">
            <Label class="text-[10px] font-bold uppercase text-muted-foreground">Caching & Expiration</Label>
            <div class="flex items-center justify-between mb-1">
              <Label class="text-[9px] text-muted-foreground">Enable Caching</Label>
              <Switch v-model="preflight.cacheToken" class="scale-75" />
            </div>
            <div v-if="preflight.cacheToken" class="space-y-2 animate-in fade-in">
              <div class="space-y-1.5">
                <Label class="text-[9px] text-muted-foreground">Duration Key / Unit</Label>
                <div class="flex gap-2">
                  <Input v-model="preflight.cacheDurationKey" class="h-7 text-[9px] font-mono flex-1"
                    placeholder="expires_in" />
                  <Select v-model="preflight.cacheDurationUnit">
                    <SelectTrigger class="h-7 w-20 text-[9px]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="seconds" class="text-[9px]">sec</SelectItem>
                      <SelectItem value="minutes" class="text-[9px]">min</SelectItem>
                      <SelectItem value="hours" class="text-[9px]">hrs</SelectItem>
                      <SelectItem value="days" class="text-[9px]">days</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
        ...
        <div class="text-[9px] text-muted-foreground italic bg-primary/5 p-2 rounded border border-primary/10">
          // This request will run automatically to obtain and refresh the authentication token.
        </div>
      </div>
    </div>
  </div>
