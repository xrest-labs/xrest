    ...
    <Dialog :open="open" @update:open="emit('update:open', $event)">
        <DialogContent class="sm:max-w-[425px]">
            <DialogHeader>
                ...
                <DialogDescription>
                    Configure the details for your new endpoint. Choose a collection or leave empty to create a new one.
                </DialogDescription>
            </DialogHeader>
            <div class="grid gap-4 py-4">
                <div class="grid grid-cols-4 items-center gap-4">
                    <Label class="text-right text-xs">Collection</Label>
                    <Select v-model="selectedCollectionId">
                        ...
                        <SelectContent>
                            <SelectGroup>
                                ...
                                <SelectItem v-for="collection in collections" :key="collection.id"
                                    :value="collection.id" class="text-xs">
                                    {{ collection.name }}
                                </SelectItem>
                            </SelectGroup>
                        </SelectContent>
                ...
                <div class="grid grid-cols-4 items-center gap-4">
                    <Label for="endpoint-name-coll" class="text-right text-xs">Name</Label>
                    <Input id="endpoint-name-coll" v-model="newEndpointName" placeholder="E.g. Get User Profile"
                        class="col-span-3 h-8 text-xs" @keyup.enter="handleAddEndpoint" />
                </div>
                <div class="grid grid-cols-4 items-center gap-4">
                ...
                <div class="grid grid-cols-4 items-center gap-4">
                    <Label for="endpoint-path-coll" class="text-right text-xs">URL/Path</Label>
                    <Input id="endpoint-path-coll" v-model="newEndpointPath"
                        placeholder="https://api.example.com/endpoint" class="col-span-3 h-8 text-xs font-mono"
